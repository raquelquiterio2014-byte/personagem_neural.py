#include <stdio.h> #include <string.h> #include <stdlib.h> #include <ctype.h>// Função para converter em minúsculas void minusculas(char *texto) {
    for (int i = 0; texto[i]; i++)
        texto[i] = tolower(texto[i]);
}
// Função para Raquel falar com voz void falar(const char *texto) {#ifdef _WIN32
    char comando[512];
    snprintf(comando, sizeof(comando), "espeak \"%s\" -v pt+f4 -s 150", texto);#elsechar comando[512];
    snprintf(comando, sizeof(comando), "espeak \"%s\" -v pt+f4 -s 150 2>/dev/null", texto);#endif    system(comando);
}
// Função para detectar emoção const char* detectar_emocao(const char *texto) {
...
// Função para responder conforme emoção const char* resposta_da_raquel(const char *texto) {
    if (strstr(texto, "oi"))
